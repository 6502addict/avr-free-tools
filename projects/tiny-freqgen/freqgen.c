...
const char str_header[]     PROGMEM = "FreqGen v1.0\r\n";
const char str_help[]       PROGMEM = "Commands: <freq> or stop\r\n";
const char str_prompt[]     PROGMEM = "> ";
const char str_backspace[]  PROGMEM = " \b";
...
static void process_command(char *cmd) {
    if (is_stop_command(cmd)) {
        freq_gen_stop();
        uart_tx_string_P(str_stopped);
    } else {
        uint16_t freq = parse_frequency(cmd);
	if (freq > 0 && freq < 20000) {
            freq_gen_set(freq);
            uart_tx_string_P(str_ok);
...
int main(void) {
    ...
    uart_tx_string_P(str_header);
    uart_tx_string_P(str_help);
    ...
    while (1) {
        ...
        if (c == '\r' || c == '\n') {
            ...
            if (cmd_index > 0) {
                cmd_index--;
                uart_tx_string_P(str_backspace);  // Erase character
            }
